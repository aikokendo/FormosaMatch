...
FLAGS = gflags.FLAGS
gflags.DEFINE_string('location', 'Taipei', 'Zip or city name')
gflags.DEFINE_integer('age_min', 21, 'Minimum age to pull')
gflags.DEFINE_integer('age_max', 35, 'Maximum age to pull')
...
def extract_usernames(html):
    ...
    if(len(scripts)>=10):
        ...
        script_pos = script.find("DeferredJS.addBundles(")
        ...
        ase = 0
        userSection = newDictionary[1]
        for userSection in userSection['params']:
            for userData in userSection['data']:
                if ase == 1:
                    try:
                        plaintext_usernames.append(str(userData['username']))
                    except UnicodeError, e:
                        print 'Skipping funky username', userData['username']
                ase = 1
